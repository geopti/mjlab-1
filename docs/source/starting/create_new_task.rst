...
This guide demonstrates how to create a 
custom task using a CartPole example. You'll 
define the robot entity, configure the MDP, 
and train a policy.
...
Define the robot model and entity configuration. 
This example uses a simple CartPole with a sliding 
base and a hinged pole.

Create the following structure in 
mjlab/src/mjlab/asset_zoo/robots:
...
.. note::
  ...
  **On Motor-Type Actuators**: You can use XML-defined actuators by leaving the 
  ``actuators`` field empty in ``EntityArticulationInfoCfg``. 
  For position control with motor-type actuators, implement 
  impedance control: ``τ = Kp * (q* - q) + Kd * (-q̇)``. See 
  https://github.com/mujocolab/mjlab/discussions/130 for details.

__init__.py
^^^^^^^^^^^

Create an empty ``__init__.py`` file to mark the directory 
as a Python package.
...
The policy outputs position commands for the cart's 
slide joint, scaled by 20.0:
...
The policy observes normalized pole angle, 
angular velocity, cart position, and
cart velocity:
